    ...
    public partial class MainWindow
    {
		private void GetCurrentInformation(TreeNode node)
		{
			...
			if (PropertySettings.hasClickablePoint == true)
			{
				...
				{
					//bool skip = false;
					if (IsWindows7() && (MainWindow.CompareElements(node.Element, uiAutomation.GetRootElement()) == true))
					{
					...
					else
					{
						...
						if (node.Element.GetClickablePoint(out clickablePoint) != 0)
						{
							...
							attribute.Tooltip = "Point on the Automation Element that can be clicked";
							this.listAttributes.Add(attribute);
						}
					}
				}
			...
			if (PropertySettings.hasControllerFor == true)
			{
				try
				{
					...
					for (int i = 0; i < array.Length; i++)
					{
						...
						attribute.Tooltip = "Array of elements for which this element serves as the controller";
			...
			if (PropertySettings.hasHasKeyboardFocus == true)
			{
				try
				{
					...
					attribute.Tooltip = "Value that indicates whether the element has keyboard focus";
					this.listAttributes.Add(attribute);
				}
				catch { }
			}
				...
				try
				{
					...
					if (hwnd != IntPtr.Zero)
					{
						...
						windowHandleString = "0x" + hwnd.ToString("X") + 
							", WindowText: \"" + sbName.ToString() + "\", WindowClass: \"" + sbClassName.ToString() + "\"";
					}
					else
					{
			...
			if (PropertySettings.hasProcessId == true)
			{
				try
				{
					...
					string processString = process.ProcessName + ".exe (PID: " + pid.ToString() + ")";
            ...
            try
            {
                ...
                attribute.Tooltip = "Control patterns that this Automation Element supports";
                this.listAttributes.Add(attribute);
            }
            catch {}
            #endregion
